...
from .dataset.fiducial1024 import Fiducial1024
...
class Fiducial2014Datamodule(LightningDataModule):
    def __init__(
        self,
        cfg: DictConfig,
        train: bool = True,
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not self.data_train and not self.data_val:
            if(self.hparams.train):
                self.data_train = Fiducial1024(self.hparams.cfg, train=True)
            self.data_val   = Fiducial1024(self.hparams.cfg, train=False)
